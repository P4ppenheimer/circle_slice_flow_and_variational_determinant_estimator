...
class _Cylindrical_Flow(torch.nn.Module):
    ...
    def __init__(self, 
        ...
        """
        Args:
            dim_base_conditioner is the dimensionality of the obersvable data x in p(z|x) 
            when replacing the variational distribution with a flow
        """
        ...
        self.add_phase_translation = add_phase_translation
        ...
        # Rotation; 
        if add_phase_translation:
            self.cond_rotation = Rotation(num_dim_data=2, 
                                          num_dim_conditioner=num_dim_data - 2 + convert_none_to_zero(num_dim_conditioner),
                                          cap_householder_refl=False)        
    ...
    def forward(self, 
        ...
        sphere = x[:, :2]
        heights = x[:, 2:].view(-1, x.shape[1] - 2) # (batch, D - 2)
        ...
        # SUBSEQUENTLY ADDED
        if inverse and self.add_phase_translation:
            sphere, sldj, conditioner = self.cond_rotation(x=sphere, 
                                                            sldj=sldj, 
        ...
        # SUBSEQUENTLY ADDED
        if not inverse and self.add_phase_translation:
            z_sphere, sldj, conditioner = self.cond_rotation(x=z_sphere, 
                                                             sldj=sldj, 
                                                             x_conditioner=conditioner, 
                                                             inverse=inverse)
    ...
    def __init__(self, 
        ...
        for flow_id in range(num_flows):
            ...
            self.scale.append(_Cylindrical_Flow(num_bins=num_bins, 
                                                ...
                                                add_phase_translation=add_phase_translation,
                                                rezero_flag=rezero_flag,
                                                num_dim_conditioner=hidden_dim_base_mlp,
                                                num_centers=num_centers))
